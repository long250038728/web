...
### agent Team
在使用`subagent`时，该子代理会把结果直接返回给主agent中，每个agent都是独立的，当一个东西交给多个agent去处理后，他们得到结果后就会马上返回，不会再继续分析，就会导致分析出来的结果是浅显的，引入agent team就是让多个subagent在获取结果后互相讨论，得出最终的结果

#### 何时使用
* 应该使用： 需要同时让多个不同维度的subagent去分析一个问题，让他们讨论出一个结果（复杂的任务，同时需要多agent互相找对方的漏洞）
* 不应该使用： 简单的任务（单agent就可以），无需多subagent互相交流（subagent就可以）
...
#### agent Team使用
无需像`subagent`那样预先定义，只需在聊天对话中说出需要使用agent team，同时需要多少个teammates，每个teammates负责做什么事，teammates完成初步分析后可以挑战彼此的结论
```
...
3. **使用 Sub-Agent 解耦复杂任务**  
   ...
   - Sub-Agent 会导致主Agent的请求消息重新再Sub-Agent询问，这部分会浪费一定的token，可以让Sub-Agent接入更便宜的大模型
   - 有效减少主上下文的信息量
