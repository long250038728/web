...
4. 快速复制表 使用mysqldump方法把数据导出insert避免原表行锁。 使用ibd的方式（云服务无法直接操作）
   * crate table r like t; //创建表r
   * alter table r discard tablespace; //删除r的空间
   * flush table t for export； //生成一个t.cfg文件  从flush table 到 unlock tables中间是会加锁的
   * cp t.cfg r.cfg; cp t.ibd r.ibd; // 拷贝
   * unlock tables; // c.cgf文件删除
   * alter table r import tablespace; // r.ibd文件作为表r的新的表空间  这个时间也会较长
5. grant授权语句（在磁盘user,db表更新，内存中找对象更新）， flush privileges语句会清空内存对象（手动操作表才要执行）
   * 全局权限: mysql.user表 &&  内存数组acl_users 。 创建连接会拷贝到自己的线程对象中 。 所以全局不对已经连接的有影响 （可能考虑到不会经常变动）
   * db权限: mysql.db表 &&  内存数组acl_dbs。会影响已经存在的链接 （需要实时校验）
   * 表权限及列权限： mysql.columns_priv表 && 内存的hash结构column_priv_hash 。会影响已经存在的链接 （需要实时校验）
6. 分区表。MDL锁力度大。
