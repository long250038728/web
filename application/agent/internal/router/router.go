...
func RegisterHTTPServer(engine *gin.Engine, srv *service.Service) {
	...
	infoGroup := engine.Group("/agent/info/").Use(interceptor.BaseHandle(cache))
	{
		infoGroup.GET("logs", func(c *gin.Context) {
			gateway.Json(c, &agent.LogsRequest{}).Use(interceptor.Limit()).Handle(func(ctx context.Context, req any) (any, error) {
				return srv.Logs(ctx, req.(*agent.LogsRequest))
			})
		})

		infoGroup.GET("events", func(c *gin.Context) {
			gateway.Json(c, &agent.EventsRequest{}).Use(interceptor.Limit()).Handle(func(ctx context.Context, req any) (any, error) {
				return srv.Events(ctx, req.(*agent.EventsRequest))
			})
		})

		infoGroup.GET("resources", func(c *gin.Context) {
			gateway.Json(c, &agent.ResourcesRequest{}).Use(interceptor.Limit()).Handle(func(ctx context.Context, req any) (any, error) {
